...
/**
 * From: https://github.com/android10/Android-CleanArchitecture-Kotlin
 *
...
sealed class Either<out L, out R> {
  ...
  /**
   * Returns the value from `Left` or null if Either is a `Right`.
   * @see Left
   */
  fun l(): L? =
    when (this) {
  ...
  /**
   * Returns the value from `Right` or null if Either is a `Left`.
   * @see Right
   */
  fun r(): R? =
    when (this) {
